...
{
  "dependencies": {
    "com.actionfit.lava-rush.installer": "https://github.com/ActionFit-Editor/LavaRushInstaller.git#0.1.5"
  }
}
```
...
- `jp.hadashikick.vcontainer@1.16.8` (UILighting 런타임 의존성)
...
프로젝트별 외형 작업이 필요하면 번들 설치가 끝난 뒤 Custom Package Manager에서 `com.actionfit.lava-rush.ui`만 **Embed for Edit**합니다. 0.1.9에는 원본 14개 역할 프리팹, 원본 PNG 56개, 필요한 시각 의존성이 그대로 들어 있으므로 정상 동작하는 베이스에서 이미지와 nested prefab을 프로젝트 스타일로 교체할 수 있습니다. AI 생성·placeholder·자동 대체 리소스는 베이스에 포함하지 않습니다. 이 전환은 자동으로 실행되지 않습니다. 호환되는 embedded UI는 설치·복구·제한 해제에서 보존되고, 요구 버전보다 오래된 embedded UI는 사용자 파일을 교체하지 않고 명시적 충돌로 남습니다. engine, Content Core, Time과 UI Foundation은 downloaded 상태를 유지합니다.

## 설치와 복구

1. installer가 독립적으로 컴파일됩니다. `package.json`의 dependencies는 비워 둡니다.
2. Custom Package Manager가 없으면 canonical `1.1.112` Git URL을 먼저 설치합니다.
